...
export default async function JourneyDashboard({
  ...
  return (
    <JourneyWorkspaceShell
      ...
      user={user}
    >
      <div className={`journey-command-page journey-${path}`}>
        {welcome === '1' ? (
        ...
        <section className="journey-command-hero" id="journey-overview">
        ...
        <section
          className="journey-attention-section"
          id="journey-attention"
          aria-labelledby="journey-attention-title"
        >
          <header>
            <div>
        ...
        <section
          className="journey-opportunities-section"
          id="journey-opportunities"
          aria-labelledby="journey-opportunities-title"
        >
          <header>
            <div>
              <span>{t('journey.opportunityKicker')}</span>
              <h2 id="journey-opportunities-title">{t(`journey.${path}.opportunityTitle`)}</h2>
            </div>
            <p>{t(`journey.${path}.opportunityLead`)}</p>
          </header>
          <div className="journey-opportunity-grid">
            <Link href={`/${seg}/${path}`} className="journey-opportunity-card primary">
              <span>
                <Icon name={path} size={22} />
              </span>
              <small>{t('journey.pathWorkspace')}</small>
              <h3>{t(path === 'work' ? 'intent.work' : 'intent.study')}</h3>
              <p>{t(`journey.${path}.pathOpportunityBody`)}</p>
              <b>{t('common.continue')} →</b>
            </Link>
            <Link href={`/${seg}/countries`} className="journey-opportunity-card">
              <span>
                <Icon name="globe" size={22} />
              </span>
              <small>{t('journey.countryFit')}</small>
              <h3>{t('guide.browseCountries')}</h3>
              <p>{t(`journey.${path}.countryOpportunityBody`)}</p>
              <b>{t('common.continue')} →</b>
            </Link>
            <Link
            ...
            >
              <span>
                <Icon name={path === 'work' ? 'search' : 'money'} size={22} />
              </span>
              <small>{t(path === 'work' ? 'workspaceNav.jobs' : 'workspaceNav.funding')}</small>
              <h3>{t(`journey.${path}.discoveryTitle`)}</h3>
              <p>{t(`journey.${path}.discoveryBody`)}</p>
              <b>{t('common.continue')} →</b>
            </Link>
          </div>
        </section>
        ...
        <section
          className="journey-workspace-section"
          id="journey-records"
          aria-labelledby="journey-tools-title"
        >
          <header>
            <div>
