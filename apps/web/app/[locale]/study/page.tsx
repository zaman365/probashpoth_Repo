...
export default async function StudyHub({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: segment } = await params;
  const locale = parseLocaleParam(segment);
  ...
  return (
    <>
      ...
      <Section surface="accent" title={t('guide.safetyTitle')} lead={t('intent.compareRiskStudy')}>
        <div className="hub-actions">
